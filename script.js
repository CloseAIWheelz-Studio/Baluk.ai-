...
const BALUK_ENGINE_ENABLED = false;
if (!BALUK_ENGINE_ENABLED) {
  if (balukEngineBtn) balukEngineBtn.remove();
  if (engineWorkspace) engineWorkspace.remove();
}
...
let modelOptionPointerHandled = false;
...
let agentVirtualTimer = null;
let agentVirtualStepTimers = [];
let agentIntroNoticeShown = false;
...
let premiumPaymentPending = false;
...
function setBalleMode(enabled = false) {
  if (enabled) agentModeEnabled = false;
...
function updatePremiumUI() {
  ...
  if (premiumOwnedLabel) premiumOwnedLabel.classList.add("hidden");
  if (premiumExpiryLabel) {
    premiumExpiryLabel.classList.add("hidden");
    premiumExpiryLabel.textContent = "";
  }
  if (premiumPendingLabel) premiumPendingLabel.classList.add("hidden");
  if (drawerPremiumOpen) {
    drawerPremiumOpen.classList.add("hidden");
    drawerPremiumOpen.disabled = true;
  }
  if (premiumBuyBtn) premiumBuyBtn.classList.add("hidden");
  if (premiumPayLinkBtn) premiumPayLinkBtn.classList.add("hidden");
  if (premiumCodeRow) premiumCodeRow.classList.add("hidden");
  if (premiumConfirmBtn) premiumConfirmBtn.disabled = true;
  if (premiumModal) premiumModal.classList.add("hidden");
  if (allowProfanityMode) allowProfanityMode.checked = allowProfanity;
  document.body.classList.toggle("premium-active", true);
  updateModelVisual();
  updateThinkingQuotaUI();
  if (window.__balukEngineV2Ready && engineModeOpen) renderEngineWorkspace();
  stopBan();
  return;
  const now = Date.now();
...
function updateThinkingPlaceholder() {
  if (!userInput) return;
  if (agentModeEnabled) {
    userInput.placeholder = "Agent Mode: bir link yapistir, yanina arastirmak istedigini yaz...";
    return;
  }
...
function getWikipediaLineLimit() {
  return thinkingModeEnabled ? 15 : 6;
}
function trimWikipediaToLines(text = "", maxLines = getWikipediaLineLimit()) {
  const clean = String(text || "")
    .replace(/\s+/g, " ")
    .replace(/\[[^\]]+\]/g, "")
    .trim();
  if (!clean) return "";
  const minLines = thinkingModeEnabled ? Math.min(10, maxLines) : 0;
  const sentences = clean
    .split(/(?<=[.!?])\s+/)
    .map((line) => line.trim())
    .filter(Boolean);
  if (minLines && sentences.length < minLines) {
    const words = clean.split(/\s+/).filter(Boolean);
    if (words.length >= minLines * 5) {
      const chunkSize = Math.max(8, Math.ceil(words.length / maxLines));
      const chunkedLines = [];
      for (let i = 0; i < words.length && chunkedLines.length < maxLines; i += chunkSize) {
        chunkedLines.push(words.slice(i, i + chunkSize).join(" "));
      }
      if (chunkedLines.length >= minLines) return chunkedLines.join("\n");
    }
  }
  const lines = [];
  const source = sentences.length ? sentences : clean.match(/.{1,150}(?:\s|$)/g) || [clean];
...
async function fetchWikipediaLongExcerpt(link, maxLines = getWikipediaLineLimit()) {
  ...
  for (const ep of endpoints) {
    try {
      ...
      if (!extract) continue;
      const excerpt = trimWikipediaToLines(extract, maxLines);
      if (excerpt) return excerpt;
    } catch {}
  }
  return null;
}
async function fetchWikipediaShortSummary(query, maxLines = getWikipediaLineLimit()) {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      return {
        ...
        link: titleInfo.link,
        summary: trimWikipediaToLines(extract, maxLines)
      };
    } catch {}
  }
  return null;
...
async function fetchWikipediaExpandedSummary(query = "", knownLink = "") {
  const maxLines = thinkingModeEnabled ? 15 : 6;
  if (knownLink) {
    const longText = await fetchWikipediaLongExcerpt(knownLink, maxLines);
    if (longText) return longText;
  }
  const short = await fetchWikipediaShortSummary(query, maxLines);
  if (!short) return "";
  const longText = await fetchWikipediaLongExcerpt(short.link, maxLines);
  return longText || short.summary;
}
...
function buildThinkingWebResponse(query, analysis, webData = {}) {
  const wikiLead = String(webData.wikiExcerpt || "").trim();
  const lead = wikiLead || summarizeLeadText(webData.summary || webData.firstDescription || "", thinkingModeEnabled ? 1400 : 620);
...
function renderWebResults(query, items, wikiExcerpt = "", wikiLink = "") {
  ...
  const coreAnswer = (leadAnswer || fallbackAnswer).trim();
...
function createGeometryWidgetState(shape) {
  const index = geometryWidgets.length;
  const meta = geometryShapeMeta[shape];
  return {
    id: `geo_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
    shape,
    x: Math.min(82, 26 + ((index % 3) * 23)),
    y: Math.min(76, 24 + (Math.floor(index / 3) * 18)),
    edges: Array(meta?.sides?.length || 0).fill(""),
    goal: "",
    rational: {
      aNum: "",
      aDen: "",
...
function syncGeometryWidgetsFromDom() {
  if (!geometrySketch || !geometryWidgets.length) return;
  geometryWidgets = geometryWidgets.map((widget) => {
    const root = geometrySketch.querySelector(`[data-widget-id="${widget.id}"]`);
    if (!root) return widget;
    if (widget.shape === "rational") {
      return {
        ...widget,
        rational: {
          aNum: root.querySelector('[data-rational="a"] .rat-num')?.value || "",
          aDen: root.querySelector('[data-rational="a"] .rat-den')?.value || "",
          bNum: root.querySelector('[data-rational="b"] .rat-num')?.value || "",
          bDen: root.querySelector('[data-rational="b"] .rat-den')?.value || "",
          op: root.querySelector(".rat-op")?.value || "+"
        }
      };
    }
    return {
      ...widget,
      edges: [...root.querySelectorAll("input[data-edge-index]")].map((input) => input.value || ""),
      goal: root.querySelector(".geo-goal-input")?.value || ""
    };
  });
}
function getSelectedGeometryWidgetState() {
  return geometryWidgets.find((widget) => widget.id === selectedGeometryWidgetId) || geometryWidgets[geometryWidgets.length - 1] || null;
}
function renderGeometryWidgetMarkup(widget) {
  if (!widget) return "";
  if (widget.shape === "rational") {
    return `
      <div id="geoWidgetWrap-${widget.id}" class="geo-widget-wrap rational-widget ${widget.id === selectedGeometryWidgetId ? "selected" : ""}" data-widget-id="${widget.id}" data-shape="rational" style="left:${widget.x}%; top:${widget.y}%">
        <button class="geo-delete-btn" type="button" data-geo-delete="${widget.id}" aria-label="Öğeyi sil">✕</button>
        <div class="rational-zone">
          ...
          <select class="rat-op" data-rat-op="${widget.id}" aria-label="Rasyonel işlem seç">
            <option value="+">+</option>
            <option value="-">−</option>
            <option value="*">×</option>
            <option value="/">÷</option>
  ...
  const meta = geometryShapeMeta[widget.shape];
  if (!meta) return "";
  const positions = sidePositions(widget.shape, meta.sides.length);
  const edges = meta.sides.map((label, idx) => `<label class="geo-side-input pos-${positions[idx] || "top"}"><span>${label}</span><input type="number" min="0" step="any" data-widget-id="${widget.id}" data-edge-index="${idx + 1}" placeholder="cm"></label>`).join("");
  return `
    <div id="geoWidgetWrap-${widget.id}" class="geo-widget-wrap geometry-widget ${widget.id === selectedGeometryWidgetId ? "selected" : ""}" data-widget-id="${widget.id}" data-shape="${widget.shape}" style="left:${widget.x}%; top:${widget.y}%">
      <button class="geo-delete-btn" type="button" data-geo-delete="${widget.id}" aria-label="Öğeyi sil">✕</button>
      <svg class="geo-shape-svg shape-${widget.shape}" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">${geometrySvgForShape(widget.shape)}</svg>
      ${edges}
      <input class="geo-goal-input" data-widget-id="${widget.id}" type="text" placeholder="alan / çevre">
    </div>
  `;
}
...
function getSelectedGeometryWidgetElement() {
  const active = getSelectedGeometryWidgetState();
  if (!active || !geometrySketch) return null;
  return geometrySketch.querySelector(`[data-widget-id="${active.id}"]`);
}
function renderGeometryWidgetMarkup(widget) {
  if (!widget) return "";
  if (widget.shape === "rational") {
    const rational = widget.rational || {};
    return `
      <div id="geoWidgetWrap-${widget.id}" class="geo-widget-wrap rational-widget ${widget.id === selectedGeometryWidgetId ? "selected" : ""}" data-widget-id="${widget.id}" data-shape="rational" style="left:${widget.x}%; top:${widget.y}%">
        <button class="geo-delete-btn" type="button" data-geo-delete="${widget.id}" aria-label="Ogeyi sil">x</button>
        <div class="rational-zone">
          <div class="rational-box" data-rational="a">
            <input class="rat-num" type="number" step="any" placeholder="ust" value="${escapeHtml(rational.aNum || "")}">
            <span class="rat-line"></span>
            <input class="rat-den" type="number" step="any" placeholder="alt" value="${escapeHtml(rational.aDen || "")}">
          </div>
          <select class="rat-op" data-rat-op="${widget.id}" aria-label="Rasyonel islem sec">
            <option value="+"${(rational.op || "+") === "+" ? " selected" : ""}>+</option>
            <option value="-"${rational.op === "-" ? " selected" : ""}>-</option>
            <option value="*"${rational.op === "*" ? " selected" : ""}>*</option>
            <option value="/"${rational.op === "/" ? " selected" : ""}>/</option>
          </select>
          <div class="rational-box" data-rational="b">
            <input class="rat-num" type="number" step="any" placeholder="ust" value="${escapeHtml(rational.bNum || "")}">
            <span class="rat-line"></span>
            <input class="rat-den" type="number" step="any" placeholder="alt" value="${escapeHtml(rational.bDen || "")}">
          </div>
        </div>
      </div>
    `;
  ...
  if (!meta) return "";
  const positions = sidePositions(widget.shape, meta.sides.length);
  const storedEdges = Array.isArray(widget.edges) ? widget.edges : [];
  const edges = meta.sides.map((label, idx) => `<label class="geo-side-input pos-${positions[idx] || "top"}"><span>${label}</span><input type="number" min="0" step="any" data-widget-id="${widget.id}" data-edge-index="${idx + 1}" placeholder="cm" value="${escapeHtml(storedEdges[idx] || "")}"></label>`).join("");
  return `
    <div id="geoWidgetWrap-${widget.id}" class="geo-widget-wrap geometry-widget ${widget.id === selectedGeometryWidgetId ? "selected" : ""}" data-widget-id="${widget.id}" data-shape="${widget.shape}" style="left:${widget.x}%; top:${widget.y}%">
      <button class="geo-delete-btn" type="button" data-geo-delete="${widget.id}" aria-label="Ogeyi sil">x</button>
      <svg class="geo-shape-svg shape-${widget.shape}" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">${geometrySvgForShape(widget.shape)}</svg>
      ${edges}
      <input class="geo-goal-input" data-widget-id="${widget.id}" type="text" placeholder="alan / cevre" value="${escapeHtml(widget.goal || "")}">
    </div>
  `;
}
function renderGeometrySketch(shape) {
  if (!geometrySketch) return;
  syncGeometryWidgetsFromDom();
  if (shape && geometryShapeMeta[shape]) {
    const widget = createGeometryWidgetState(shape);
    geometryWidgets.push(widget);
    selectedGeometryWidgetId = widget.id;
    selectedGeometryShape = shape;
    geometryPlacement = { x: widget.x, y: widget.y };
  }
  renderNotebookShell(geometryWidgets.map((widget) => renderGeometryWidgetMarkup(widget)).join(""));
  wireGeometryInputRules();
  wireGeometryDrag();
  wireNotebookItemSelection();
}
function wireNotebookItemSelection() {
  const wraps = [...(geometrySketch?.querySelectorAll(".geo-widget-wrap") || [])];
  wraps.forEach((wrap) => {
    if (wrap.dataset.selectBound === "1") return;
    wrap.dataset.selectBound = "1";
    wrap.addEventListener("click", (e) => {
      e.stopPropagation();
      selectedGeometryWidgetId = wrap.dataset.widgetId || null;
      selectedGeometryShape = wrap.dataset.shape || null;
      geometrySketch?.querySelectorAll(".geo-widget-wrap").forEach((node) => {
        node.classList.toggle("selected", node.dataset.widgetId === selectedGeometryWidgetId);
      });
    });
  });
  geometrySketch?.querySelectorAll("[data-geo-delete]").forEach((del) => {
    if (del.dataset.bound === "1") return;
    del.dataset.bound = "1";
    del.addEventListener("click", (e) => {
      e.stopPropagation();
      const widgetId = del.getAttribute("data-geo-delete");
      geometryWidgets = geometryWidgets.filter((widget) => widget.id !== widgetId);
      const active = geometryWidgets[geometryWidgets.length - 1] || null;
      selectedGeometryWidgetId = active?.id || null;
      selectedGeometryShape = active?.shape || null;
      if (active) geometryPlacement = { x: active.x, y: active.y };
      renderGeometrySketch(null);
      clearGeometryWarn();
    });
  });
...
function wireGeometryDrag() {
  ...
  const move = (ev) => {
    ...
    const dx = ((point.clientX - geometryDragState.startX) / rect.width) * 100;
    const dy = ((point.clientY - geometryDragState.startY) / rect.height) * 100;
    const nextX = Math.min(84, Math.max(16, geometryDragState.baseX + dx));
    const nextY = Math.min(82, Math.max(20, geometryDragState.baseY + dy));
    const active = geometryWidgets.find((widget) => widget.id === geometryDragState.widgetId);
    if (active) {
      active.x = nextX;
      active.y = nextY;
    }
    geometryPlacement = { x: nextX, y: nextY };
    const wrap = geometrySketch?.querySelector(`[data-widget-id="${geometryDragState.widgetId}"]`);
    if (wrap) {
      wrap.style.left = `${nextX}%`;
      wrap.style.top = `${nextY}%`;
    }
  };
  const end = () => {
    if (!geometryDragState) return;
  ...
  geometrySketch?.querySelectorAll(".geo-widget-wrap").forEach((wrap) => {
    wrap.onmousedown = (ev) => {
      const point = ev.touches ? ev.touches[0] : ev;
      const active = geometryWidgets.find((widget) => widget.id === wrap.dataset.widgetId);
      geometryDragState = {
        widgetId: wrap.dataset.widgetId,
        startX: point.clientX,
        startY: point.clientY,
        baseX: active?.x ?? geometryPlacement.x,
        baseY: active?.y ?? geometryPlacement.y
      };
      selectedGeometryWidgetId = wrap.dataset.widgetId || null;
      selectedGeometryShape = wrap.dataset.shape || null;
      wrap.classList.add("dragging");
    ...
    wrap.ontouchstart = wrap.onmousedown;
  });
  if (geometryDragMoveHandler) {
    document.removeEventListener("mousemove", geometryDragMoveHandler);
    document.removeEventListener("touchmove", geometryDragMoveHandler);
...
function getShapeEdges(root = geometrySketch) {
  if (!root) return [];
  const inputs = [...root.querySelectorAll("input[data-edge-index]")];
  return inputs.map((i) => parsePositive(i.value));
}
function wireGeometryInputRules() {
  if (!geometrySketch) return;
  geometrySketch.querySelectorAll(".geometry-widget").forEach((widget) => {
    if (widget.dataset.inputBound === "1") return;
    widget.dataset.inputBound = "1";
    const shape = widget.dataset.shape;
    const inputs = [...widget.querySelectorAll("input[data-edge-index]")];
    inputs.forEach((input, idx) => {
      input.addEventListener("input", () => {
        clearGeometryWarn();
        const v = input.value;
...
function computeRationalCard(root = geometrySketch) {
  const boxes = [...(root?.querySelectorAll(".rational-box") || [])];
  if (boxes.length < 2) return { error: "Ä°ki rasyonel sayÄ± gerekli." };
  const [a, b] = boxes.map((box) => ({
    n: parseSigned(box.querySelector(".rat-num")?.value),
    d: parseSigned(box.querySelector(".rat-den")?.value)
  }));
  if (a.n === null || a.d === null || b.n === null || b.d === null) return { error: "Pay/payda alanlarÄ±nÄ± doldur." };
  if (a.d === 0 || b.d === 0) return { error: "Payda 0 olamaz." };
  const op = root?.querySelector(".rat-op")?.value || "+";
  let n = 0;
  let d = 1;
  if (op === "+") { n = a.n * b.d + b.n * a.d; d = a.d * b.d; }
  if (op === "-") { n = a.n * b.d - b.n * a.d; d = a.d * b.d; }
  if (op === "*") { n = a.n * b.n; d = a.d * b.d; }
  if (op === "/") {
    if (b.n === 0) return { error: "SÄ±fÄ±ra bÃ¶lme yapÄ±lamaz." };
    n = a.n * b.d; d = a.d * b.n;
  }
  const signFix = d < 0 ? -1 : 1;
  n *= signFix;
  ...
  return {
    label: "Rasyonel SonuÃ§",
    value: `${n / gcd(n, d)}/${d / gcd(n, d)}`,
    formula: `(${a.n}/${a.d}) ${op} (${b.n}/${b.d}) = ${n / gcd(n, d)}/${d / gcd(n, d)}`,
    unit: ""
  };
}
function computeGeometry(shape, root = geometrySketch) {
  const edges = getShapeEdges(root);
  const goal = normalizeGoal(root?.querySelector(".geo-goal-input")?.value || "");
  if (!goal) return { error: "Ortadaki kutuya alan veya Ã§evre yaz." };
  if (edges.some((v) => v === null)) return { error: "TÃ¼m gerekli kenar/r deÄŸerlerini pozitif cm olarak gir." };
  if (shape === "square") {
    const a = edges[0];
    return goal === "alan"
      ? { value: a * a, formula: `${a} x ${a} = ${a * a}`, unit: "cmÂ²", label: "Alan" }
      : { value: 4 * a, formula: `${a} x 4 = ${4 * a}`, unit: "cm", label: "Ã‡evre" };
  }
  if (shape === "rectangle") {
    const long = edges[0];
    const short = edges[1];
    if (!(long > short)) return { error: "DikdÃ¶rtgende uzun kenar kÄ±sa kenardan bÃ¼yÃ¼k olmalÄ±." };
    const area = long * short;
    const perimeter = 2 * (long + short);
    return goal === "alan"
      ? { value: area, formula: `${long} x ${short} = ${area}`, unit: "cmÂ²", label: "Alan" }
      : { value: perimeter, formula: `2 x (${long} + ${short}) = ${perimeter}`, unit: "cm", label: "Ã‡evre" };
  }
  if (shape === "triangle") {
    const [a, b, c] = edges;
    const p = a + b + c;
    if (goal === "Ã§evre") return { value: p, formula: `${a}+${b}+${c} = ${p}`, unit: "cm", label: "Ã‡evre" };
    const s = p / 2;
    const area2 = s * (s - a) * (s - b) * (s - c);
    if (area2 <= 0) return { error: "Bu kenarlarla geÃ§erli Ã¼Ã§gen oluÅŸmuyor." };
    const area = Number(Math.sqrt(area2).toFixed(2));
    return { value: area, formula: `âˆš(s(s-a)(s-b)(s-c)) = ${area}`, unit: "cmÂ²", label: "Alan" };
  }
  if (shape === "circle") {
    const r = edges[0];
    const area = Number((Math.PI * r * r).toFixed(2));
    const perimeter = Number((2 * Math.PI * r).toFixed(2));
    return goal === "alan"
      ? { value: area, formula: `Ï€ x ${r}Â² = ${area}`, unit: "cmÂ²", label: "Alan" }
      : { value: perimeter, formula: `2Ï€ x ${r} = ${perimeter}`, unit: "cm", label: "Ã‡evre" };
  }
  if (shape === "parallelogram") {
    const a = edges[0];
    const b = edges[1];
    ...
    const perimeter = 2 * (a + b);
    return goal === "alan"
      ? { value: area, formula: `${a} x ${b} = ${area}`, unit: "cmÂ²", label: "Alan" }
      : { value: perimeter, formula: `2 x (${a}+${b}) = ${perimeter}`, unit: "cm", label: "Ã‡evre" };
  }
  if (shape === "trapezoid") {
    const [a, b, c, d] = edges;
    const perimeter = a + b + c + d;
    if (goal === "Ã§evre") return { value: perimeter, formula: `${a}+${b}+${c}+${d} = ${perimeter}`, unit: "cm", label: "Ã‡evre" };
    const h = Number(Math.abs(a - b).toFixed(2));
    const area = Number((((a + b) / 2) * h).toFixed(2));
    return { value: area, formula: `((${a}+${b})/2) x ${h} = ${area}`, unit: "cmÂ²", label: "Alan" };
  }
  if (shape === "pentagon" || shape === "hexagon") {
    const n = shape === "pentagon" ? 5 : 6;
    const side = edges[0];
    const perimeter = n * side;
    if (goal === "Ã§evre") return { value: perimeter, formula: `${side} x ${n} = ${perimeter}`, unit: "cm", label: "Ã‡evre" };
    const area = Number(((n * side * side) / (4 * Math.tan(Math.PI / n))).toFixed(2));
    return { value: area, formula: `dÃ¼zgÃ¼n ${n}gen alan formÃ¼lÃ¼ = ${area}`, unit: "cmÂ²", label: "Alan" };
  }
  return { error: "Bu ÅŸekil iÃ§in hesaplama tanÄ±mlanamadÄ±." };
}
function solveGeometryCard() {
  clearGeometryWarn();
  const active = getSelectedGeometryWidgetState();
  ...
  const activeElement = getSelectedGeometryWidgetElement();
  if (!activeElement) return;
  selectedGeometryShape = active.shape;
  const result = active.shape === "rational"
    ? computeRationalCard(activeElement)
    : computeGeometry(active.shape, activeElement);
  if (result.error) {
    showGeometryWarn(result.error);
    return;
  }
  ...
  if (resultPad) resultPad.textContent = `${result.label} = ${result.value}${result.unit ? " " + result.unit : ""}`;
  showGeometrySolveEffect();
  const shapeLabel = geometryShapeMeta[active.shape]?.label || "Matematik";
  const userMsg = `Geometri/Matematik: ${shapeLabel} (${result.label})`;
  const botMsg = `ğŸ§© ${shapeLabel} ${result.label} = ${result.value}${result.unit ? " " + result.unit : ""}
AdÄ±m: ${result.formula}`;
  startChatIfNeeded();
  addMessage(userMsg, "user");
  const thinking = addThinkingBubble("math");
  setTimeout(() => fillThinkingBubble(thinking, botMsg, "Ä°ÅŸlem analiz edildi â€¢ sonuÃ§ hazÄ±r âœ…"), 3000);
}
const tutorSteps = [
  {
...
function updateThinkingStatus(node, status) {
  ...
  const statusEl = node.querySelector(".thinking-status-stack span") || node.querySelector(".thinking-head > span");
...
function handlePlusToggleClick() {
  if (!plusToggle || !plusMenu) return;
  if (thinkingModeEnabled) {
...
function handleMemoryToggleClick() {
...
function handleThinkingToggleClick() {
  setThinkingMode(!thinkingModeEnabled);
}
function handleThinkingPromoCloseClick() {
  hideThinkingPromo();
}
function handleDrawerCloseClick() {
  if (sideDrawer) sideDrawer.classList.add("hidden");
}
function handleDrawerAccountSettingsClick() {
  if (!isAccountLoggedIn) {
    showWarningOverlay("Önce oturum aç.");
    return;
  }
...
function handleNewChatClick() {
  createAndSwitchNewChat();
}
function handleDrawerPremiumOpenClick() {
  if (premiumModal && !isPremiumUser) premiumModal.classList.remove("hidden");
}
function handleDrawerBackgroundOpenClick() {
  if (backgroundModal) backgroundModal.classList.remove("hidden");
  if (sideDrawer) sideDrawer.classList.add("hidden");
}
function handleUiSettingsOpenClick() {
  openUiSettingsModal();
}
function handleUiSettingsCloseClick() {
  closeUiSettingsModal();
}
function handleVoiceCloseClick() {
  closeVoiceMode();
}
function handleVoiceWebClick() {
...
function handleVoiceMuteClick() {
  voiceOutputMuted = !voiceOutputMuted;
  if (voiceMuteBtn) voiceMuteBtn.classList.toggle("muted", voiceOutputMuted);
...
function handleModelToggleClick() {
  if (!modelMenu) return;
  if (!isAccountLoggedIn) {
...
function handleMathStudioToggleClick() {
  if (mathStudioPanel) mathStudioPanel.classList.toggle("hidden");
}
...
function handleBackgroundCloseClick() {
  if (backgroundModal) backgroundModal.classList.add("hidden");
}
function handlePremiumCloseClick() {
  if (premiumModal) premiumModal.classList.add("hidden");
}
function handleLensCloseClick() {
  if (balukLensMode) balukLensMode.checked = false;
  setLensMode(false);
}
function handleLensPickClick() {
  if (lensFileInput) lensFileInput.click();
}
function handleLensAnalyzeClick() {
  runLensAnalysis();
}
function handleSaveAccountClick() {
  saveAccountProfile();
}
function handlePersistAccountClick() {
  const profile = sanitizeAccountProfile({
    name: accountName?.value,
    gmail: accountGmail?.value,
    photo: accountPhoto?.value
...
function handleLogoutAccountClick() {
  logoutAccountProfile();
}
function handleDeleteAccountClick() {
  deleteAccountProfile();
}
function handlePremiumBuyClick() {
  startPremiumPayment();
}
function handlePremiumPayLinkClick() {
  openPremiumPaymentLink();
}
function handlePremiumConfirmClick() {
  manuallyConfirmPremium();
}
function handleThinkingUpgradeClick() {
  if (premiumModal) premiumModal.classList.remove("hidden");
}
function handleThinkingDismissClick() {
  hideThinkingLimitBanner();
}
function handleCloseSafetySurveyClick() {
  closeSafetySurvey();
}
function handleBanUnlockClick() {
  if (banPassword && banPassword.value.trim() === "baluk2026") {
    stopBan();
    banPassword.value = "";
  }
}
function handleEngineBackClick() {
  closeBalukEngine();
}
function handleEngineThemeClick() {
  openEngineSettingsPanel();
}
function handleEngineAuthLoginClick() {
  closeBalukEngine();
  if (accountPanel) accountPanel.classList.remove("hidden");
  if (sideDrawer) sideDrawer.classList.add("hidden");
}
function handleEngineCodeToggleClick() {
  toggleEnginePanel("code");
}
function handleEngineRunToggleClick() {
  toggleEnginePanel("run");
}
function handleEngineDockCloseClick() {
  toggleEnginePanel(enginePanelMode);
}
function handleEngineSettingsCloseClick() {
  closeEngineSettingsPanel();
}
function handleEngineIdeasToggleClick() {
  toggleEngineIdeas();
}
function handleEngineSubmitClick() {
  submitEnginePrompt();
}
function handleEngineQuotaUpgradeClick() {
  closeEngineQuotaPanel();
  if (premiumModal) premiumModal.classList.remove("hidden");
}
function handleEngineQuotaDismissClick() {
  closeEngineQuotaPanel();
}
function handleSolveGeometryClick() {
  solveGeometryCard();
}
function handleMathTutorNextClick() {
  tutorStep = Math.min(tutorStep + 1, tutorSteps.length - 1);
  renderTutorStep();
}
function handleMathTutorSkipClick() {
...
function handleMathTutorDoneClick() {
  if (mathTutorOverlay) mathTutorOverlay.classList.add("hidden");
  clearTutorSpotlight();
  if (plusMenu) plusMenu.classList.add("hidden");
  if (isAccountLoggedIn) {
...
function handleEngineModelPickerClick() {
  if (!engineModelMenu) return;
  engineModelMenu.classList.toggle("hidden");
  if (engineLanguageMenu) engineLanguageMenu.classList.add("hidden");
}
function handleEngineLanguagePickerClick() {
  if (!engineLanguageMenu) return;
  engineLanguageMenu.classList.toggle("hidden");
  if (engineModelMenu) engineModelMenu.classList.add("hidden");
}
function handleThinkingUnlockClick() {
  if (thinkingUnlockBtn) thinkingUnlockBtn.classList.add("hidden");
}
function ensureDrawerSettingsButton() {
  if (document.getElementById("uiSettingsToggle")) return;
  const toolsSection = document.querySelector(".drawer-tools-section");
  const toolsHeading = toolsSection?.querySelector("h4");
  if (!toolsSection || !toolsHeading) return;
  const button = document.createElement("button");
  button.id = "uiSettingsToggle";
  button.type = "button";
  button.className = "drawer-btn";
  button.innerHTML = `<span class="menu-icon icon-gear" aria-hidden="true"></span><span>Ayarlar</span>`;
  toolsHeading.insertAdjacentElement("afterend", button);
}
function ensureDrawerBalosButton() {
  if (document.getElementById("drawerBalosSoon")) return;
  const toolsSection = document.querySelector(".drawer-tools-section");
  const premiumButton = document.getElementById("drawerPremiumOpen");
  if (!toolsSection) return;
  const button = document.createElement("button");
  button.id = "drawerBalosSoon";
  button.type = "button";
  button.className = "drawer-btn drawer-btn-soon";
  button.disabled = true;
  button.setAttribute("aria-disabled", "true");
  button.title = "Yakinda gelecek";
  button.innerHTML = `<span class="menu-icon icon-tools" aria-hidden="true"></span><span>BalOS</span><small>Yakinda gelecek</small>`;
  if (premiumButton) premiumButton.insertAdjacentElement("beforebegin", button);
  else toolsSection.appendChild(button);
}
function ensureAgentModeButton() {
  if (!plusMenu || document.getElementById("agentModeBtn")) return;
  const button = document.createElement("button");
  button.id = "agentModeBtn";
  button.type = "button";
  button.className = "image-mode-label agent-mode-btn";
  button.innerHTML = `
    <span class="mode-icon agent-mode-icon" aria-hidden="true">
      <span class="agent-orbit"></span>
      <span class="agent-core">&lt;&gt;</span>
    </span>
    <span class="math-mode-pill">Agent Mode</span>
    <span class="mode-spark" aria-hidden="true">beta</span>
  `;
  const webLabel = plusMenu.querySelector(".web-mode-label");
  if (webLabel) webLabel.insertAdjacentElement("afterend", button);
  else plusMenu.appendChild(button);
  button.addEventListener("click", () => {
    if (plusMenu) plusMenu.classList.add("hidden");
    openAgentIntroNotice();
  });
}
function ensureBalosNotice() {
  let notice = document.getElementById("balosNotice");
  if (notice) return notice;
  notice = document.createElement("div");
  notice.id = "balosNotice";
  notice.className = "balos-notice hidden";
  notice.setAttribute("role", "dialog");
  notice.setAttribute("aria-live", "polite");
  notice.setAttribute("aria-label", "BalOS bildirimi");
  notice.innerHTML = `
    <div class="balos-notice-card">
      <div class="balos-notice-kicker">Yeni</div>
      <strong>BalOS</strong>
      <p>Baluk.ai ile entegre isletim sistemi yakinda geliyor.</p>
      <button id="balosNoticeClose" type="button">Tamam</button>
    </div>
  `;
  document.body.appendChild(notice);
  notice.querySelector("#balosNoticeClose")?.addEventListener("click", () => {
    notice.classList.add("hidden");
  });
  notice.addEventListener("click", (event) => {
    if (event.target === notice) notice.classList.add("hidden");
  });
  return notice;
}
function scheduleBalosNotice() {
  if (balosNoticeShown) return;
  const notice = ensureBalosNotice();
  if (!notice) return;
  if (balosNoticeTimer) window.clearTimeout(balosNoticeTimer);
  balosNoticeTimer = window.setTimeout(() => {
    balosNoticeShown = true;
    notice.classList.remove("hidden");
  }, 5000);
}
function ensureAgentIntroNotice() {
  let notice = document.getElementById("agentIntroNotice");
  if (notice) return notice;
  notice = document.createElement("div");
  notice.id = "agentIntroNotice";
  notice.className = "agent-intro-notice hidden";
  notice.setAttribute("role", "dialog");
  notice.setAttribute("aria-live", "polite");
  notice.setAttribute("aria-label", "Yeni Agent Mode");
  notice.innerHTML = `
    <div class="agent-intro-card">
      <div class="agent-thumb" aria-hidden="true">
        <div class="agent-thumb-browser">
          <span></span><span></span><span></span>
          <div class="agent-thumb-url"></div>
          <div class="agent-thumb-scan"></div>
        </div>
        <div class="agent-thumb-mark">&lt;&gt;</div>
      </div>
      <div class="agent-intro-copy">
        <span>Yeni Agent Mode</span>
        <strong>Sanal tarayici ile siteyi izleyelim.</strong>
        <p>Linki alir, sanal masaustu ortaminda yeni sekme gibi acar, metinleri okuyor hissiyle analiz eder ve sitenin ne ise yaradigini soyler.</p>
      </div>
      <div class="agent-intro-actions">
        <button id="agentTryNowBtn" type="button">Simdi dene</button>
        <button id="agentLaterBtn" type="button">Daha sonra</button>
      </div>
    </div>
  `;
  document.body.appendChild(notice);
  notice.querySelector("#agentTryNowBtn")?.addEventListener("click", () => {
    notice.classList.add("hidden");
    setAgentMode(true);
  });
  notice.querySelector("#agentLaterBtn")?.addEventListener("click", () => {
    notice.classList.add("hidden");
  });
  notice.addEventListener("click", (event) => {
    if (event.target === notice) notice.classList.add("hidden");
...
function openAgentIntroNotice() {
  agentIntroNoticeShown = true;
  ensureAgentIntroNotice()?.classList.remove("hidden");
}
function scheduleAgentIntroNotice() {
  if (agentIntroNoticeShown) return;
  window.setTimeout(() => {
    if (!agentIntroNoticeShown) openAgentIntroNotice();
  }, 2000);
}
function ensureAgentVirtualDesktop() {
  let panel = document.getElementById("agentVirtualDesktop");
  if (panel) return panel;
  panel = document.createElement("section");
  panel.id = "agentVirtualDesktop";
  panel.className = "agent-virtual-desktop hidden";
  panel.setAttribute("aria-label", "Agent Mode sanal masaustu");
  panel.innerHTML = `
    <div class="agent-desktop-window">
      <header class="agent-window-bar">
        <div class="agent-window-dots" aria-hidden="true"><span></span><span></span><span></span></div>
        <div id="agentAddressBar" class="agent-address-bar">about:blank</div>
      </header>
      <div class="agent-browser-body">
        <div class="agent-search-stage">
          <div class="agent-search-logo">&lt;/&gt;</div>
          <div class="agent-search-title">
            <strong>Baluk Screatch</strong>
            <span>Agent Mode</span>
          </div>
          <div id="agentSearchBox" class="agent-search-box">Link bekleniyor</div>
          <div class="agent-search-hint">sanal tarayici hazirlaniyor</div>
        </div>
        <iframe id="agentLiveFrame" class="agent-live-frame" title="Agent Mode sanal sayfa" sandbox="allow-same-origin allow-scripts"></iframe>
        <div class="agent-page-skeleton">
          <div class="agent-page-hero"></div>
          <div class="agent-page-lines">
            <span></span><span></span><span></span><span></span><span></span>
          </div>
          <div class="agent-page-grid"><span></span><span></span><span></span></div>
        </div>
        <div id="agentScanBeam" class="agent-scan-beam"></div>
        <div id="agentCursor" class="agent-cursor" aria-hidden="true"></div>
      </div>
      <footer id="agentDesktopStatus" class="agent-desktop-status">Agent beklemede</footer>
    </div>
  `; 
...
function setAgentMode(enabled) {
  agentModeEnabled = Boolean(enabled);
  if (agentModeEnabled) {
    if (webSearchMode) webSearchMode.checked = false;
    setWebMode(false);
    setLensMode(false);
  ...
  if (userInput) {
    userInput.placeholder = agentModeEnabled
      ? "Agent Mode: bir link yapistir, yanina arastirmak istedigini yaz..."
      : "Mesajini yaz...";
    userInput.focus();
  }
  const btn = document.getElementById("agentModeBtn");
  if (btn) btn.classList.toggle("active", agentModeEnabled);
}
function extractAgentUrlAndQuestion(text = "") {
  const raw = String(text || "").trim();
  const urlMatch = raw.match(/https?:\/\/[^\s]+|(?:www\.)[^\s]+|[a-z0-9-]+(?:\.[a-z]{2,}){1,}[^\s]*/i);
  const originalUrl = urlMatch?.[0] || "";
  const url = originalUrl
    ? (originalUrl.startsWith("http") ? originalUrl : `https://${originalUrl}`)
    : "";
  const question = originalUrl ? raw.replace(originalUrl, "").trim() : raw;
  return { url, question };
}
function describeAgentSite(url = "", question = "") {
  let parsed = null;
  try { parsed = new URL(url); } catch {}
  const host = (parsed?.hostname || url || "bu site").replace(/^www\./, "");
  const path = parsed?.pathname || "";
  const lower = `${host} ${path} ${question}`.toLowerCase();
  let purpose = "bu adres bir web sayfasi gibi gorunuyor; ana icerik, baglanti basligi ve sayfa metinleri uzerinden genel amacini anlamaya calistim";
  if (/youtube|youtu\.be/.test(lower)) purpose = "video icerikleri, kanallar ve izleme sayfalari icin kullanilan bir platform";
  else if (/github/.test(lower)) purpose = "kod depolari, proje dosyalari, issue ve gelistirme akisi icin kullanilan bir gelistirici platformu";
  else if (/wikipedia/.test(lower)) purpose = "ansiklopedi tarzi bilgi maddeleri sunan bir kaynak";
  else if (/google|bing|duckduckgo/.test(lower)) purpose = "arama yapip farkli kaynaklara ulasmak icin kullanilan bir arama sayfasi";
  else if (/shop|store|amazon|trendyol|hepsiburada|n11|sepet|product/.test(lower)) purpose = "urun listeleme, fiyat inceleme ve satin alma akisi olan bir alisveris sayfasi";
  else if (/news|haber|gazete|cnn|bbc|ntv|hurriyet|sozcu/.test(lower)) purpose = "haber ve guncel icerik okumaya yarayan bir yayin sayfasi";
  else if (/docs|documentation|developer|api/.test(lower)) purpose = "dokumantasyon, teknik aciklama veya gelistirici rehberi sunan bir sayfa";
  const focus = question
    ? `Ozellikle "${question}" kismini arastirma notu olarak ayirdim.`
    : "Ek bir arastirma notu verilmedigi icin sayfanin genel amacina odaklandim.";
  return `Agent Mode ${host} adresini acti.\n\nGordugum kadariyla bu site ${purpose}.\n\n${focus}`;
}
function runAgentVirtualDesktop(url = "", question = "") {
  const panel = ensureAgentVirtualDesktop();
  const address = panel.querySelector("#agentAddressBar");
  const status = panel.querySelector("#agentDesktopStatus");
  const scan = panel.querySelector("#agentScanBeam");
  const cursor = panel.querySelector("#agentCursor");
  const searchBox = panel.querySelector("#agentSearchBox");
  const frame = panel.querySelector("#agentLiveFrame");
  panel.classList.remove("hidden");
  panel.dataset.locked = "1";
  panel.dataset.phase = "boot";
  panel.dataset.live = "0";
  if (frame) {
    frame.removeAttribute("src");
    frame.classList.remove("visible");
  }
  if (searchBox) searchBox.textContent = "Link bekleniyor";
  if (scan) {
  ...
  const steps = [
    { t: 0, phase: "boot", text: "Sanal masaustu aciliyor...", address: "about:blank" },
    { t: 1600, phase: "newtab", text: "Yeni sekme hazirlaniyor...", address: "new-tab://agent" },
    { t: 3200, phase: "search", text: "Link arama alanina yaziliyor...", searchType: true },
    { t: 7200, phase: "address", text: "Imlec arama cubuguna tikliyor...", type: true },
    { t: 10200, phase: "enter", text: "Link yazildi, siteye giris yapiliyor...", address: url, open: true },
    { t: 11200, phase: "clickHero", text: "Sayfa acildi, ust bolum inceleniyor...", address: url },
    { t: 15400, phase: "clickText", text: "Imlec metin bloklarini tarayip tikliyor...", address: url },
    { t: 19800, phase: "scroll", text: "Metinler okunuyor, sayfa asagi kaydiriliyor...", address: url },
    { t: 24400, phase: "cards", text: "Baglanti ve kart bolumleri karsilastiriliyor...", address: url },
    { t: 28600, phase: "done", text: question ? "Istenen arastirma notu ayriliyor..." : "Site amaci ozetleniyor...", address: url }
  ];
  if (agentVirtualTimer) window.clearTimeout(agentVirtualTimer);
  agentVirtualStepTimers.forEach((timerId) => window.clearTimeout(timerId));
  agentVirtualStepTimers = [];
  steps.forEach((step) => {
    const timerId = window.setTimeout(() => {
      panel.dataset.phase = step.phase;
      if (status) status.textContent = step.text;
      if (step.searchType) typeSearch(url);
      if (step.type) typeAddress(url);
      else if (address) address.textContent = step.address || url;
      if (step.open) {
    ...
    agentVirtualStepTimers.push(timerId);
  });
  agentVirtualTimer = window.setTimeout(() => {
    if (status) status.textContent = "Agent Mode ozeti hazir";
...
function processAgentModeInput(text = "") {
  const { url, question } = extractAgentUrlAndQuestion(text);
  startChatIfNeeded();
  addMessage(text, "user");
  if (!url) {
    addMessage("Agent Mode icin once bir link gonder. Ornek: https://example.com bu site ne ise yariyor", "bot");
    return;
  }
  const thinking = addThinkingBubble("web");
  updateThinkingStatus(thinking, "Düşünme başladı...");
  setTimeout(() => updateThinkingStatus(thinking, "Linke giris yapıyorum..."), 900);
  runAgentVirtualDesktop(url, question);
  setTimeout(() => updateThinkingStatus(thinking, "Arama cubuguna link yaziliyor..."), 4200);
  setTimeout(() => updateThinkingStatus(thinking, "Sanal tarayicida metinler okunuyor..."), 13200);
  setTimeout(() => updateThinkingStatus(thinking, "Sayfa bolumleri karsilastiriliyor..."), 22400);
  setTimeout(() => {
    fillThinkingBubble(thinking, describeAgentSite(url, question), "Agent Mode analizi hazır");
    setAgentMode(false);
  }, 30500);
}
function handleModelOptionDirectClick(optionBtn) {
  if (!optionBtn) return;
  if (advancedMathEnabled) return;
  modelOptions.forEach((i) => i.classList.remove("active"));
  optionBtn.classList.add("active");
  currentModel = optionBtn.dataset.model;
...
document.addEventListener("pointerdown", (event) => {
  ...
  if (!option || !modelMenu?.contains(option)) return;
  event.preventDefault();
  event.stopImmediatePropagation();
  modelOptionPointerHandled = true;
  handleModelOptionDirectClick(option);
  window.setTimeout(() => {
    modelOptionPointerHandled = false;
  }, 0);
}, true);
...
document.addEventListener("click", (event) => {
  ...
  if (!(target instanceof Element)) return;
  const option = target.closest(".model-option");
  if (option && modelMenu?.contains(option)) {
    event.preventDefault();
    event.stopImmediatePropagation();
    if (!modelOptionPointerHandled) handleModelOptionDirectClick(option);
    return;
  }
  const toggle = target.closest("#modelToggle");
  if (!toggle) return;
  event.preventDefault();
  event.stopImmediatePropagation();
  handleModelToggleClick();
}, true);
function handleChatListDelegatedClick(targetBtn) {
  if (!targetBtn || !chatList) return false;
  const row = targetBtn.closest(".chat-list-item");
  if (!row) return false;
  const rows = [...chatList.querySelectorAll(".chat-list-item")];
  const rowIndex = rows.indexOf(row);
  const orderedSessions = chatSessions.slice().sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
  const session = orderedSessions[rowIndex];
  if (!session) return false;
  if (targetBtn.classList.contains("chat-open-btn")) {
    switchToChatSession(session.id);
    return true;
  }
  if (targetBtn.classList.contains("chat-item-menu-btn")) {
    const menu = row.querySelector(".chat-item-menu");
    if (!menu) return true;
    chatList.querySelectorAll(".chat-item-menu").forEach((m) => m !== menu && m.classList.add("hidden"));
    menu.classList.toggle("hidden");
  ...
  if (targetBtn.matches(".chat-item-menu button[data-action]")) {
    const menu = row.querySelector(".chat-item-menu");
...
document.addEventListener("click", (e) => {
  const actionBtn = e.target.closest("#enterAppBtn, #memoryToggle, #mathStudioToggle, #uiSettingsToggle, #clearChat, #accountToggle, #drawerClose, #newChatBtn, #drawerAccountSettings, #drawerBackgroundOpen, #drawerPremiumOpen, #clearMemory, #solveGeometryBtn, #mathTutorNext, #mathTutorSkip, #mathTutorDone, #closeSafetySurveyModal, #backgroundClose, #premiumClose, #saveAccount, #persistBrowserAccount, #logoutAccount, #deleteAccount, #lensClose, #lensPickBtn, #lensAnalyzeBtn, #banUnlockBtn, #plusToggle, #balukEngineBtn, #thinkingPromoClose, #thinkingToggle, #thinkingUnlockBtn, #thinkingUpgradeBtn, #thinkingDismissBtn, #engineBackBtn, #engineThemeToggle, #engineAuthLoginBtn, #engineModelPicker, #engineLanguagePicker, #engineIdeasToggle, #engineCodeToggle, #engineRunToggle, #engineSubmitBtn, #engineDockClose, #engineSettingsClose, #engineQuotaUpgradeBtn, #engineQuotaDismissBtn, #voiceMuteBtn, #voiceWebBtn, #voiceCloseBtn, #premiumBuyBtn, #premiumPayLinkBtn, #premiumConfirmBtn, #uiModeLightBtn, #uiModeDarkBtn, #engineModeLightBtn, #engineModeDarkBtn");
  if (!actionBtn) {
    const genericBtn = e.target.closest("[data-engine-model],[data-engine-language],.chat-open-btn,.chat-item-menu-btn,.chat-item-menu button[data-action],#chatSubmitBtn,#thinkingImageRemove");
    if (!genericBtn) return;
    e.preventDefault();
    e.stopPropagation();
    if (genericBtn.matches("[data-engine-model]")) selectEngineModel(genericBtn.dataset.engineModel);
    if (genericBtn.matches("[data-engine-language]")) selectEngineLanguage(genericBtn.dataset.engineLanguage);
    if (genericBtn.id === "chatSubmitBtn") {
    ...
    if (genericBtn.id === "thinkingImageRemove") clearThinkingImageAttachment();
    handleChatListDelegatedClick(genericBtn);
    return;
  }
  e.preventDefault();
  e.stopPropagation();
  if (actionBtn.id === "enterAppBtn") openAppWithTransition();
  if (actionBtn.id === "plusToggle") handlePlusToggleClick();
  if (actionBtn.id === "clearChat") handleClearChatClick();
  if (actionBtn.id === "memoryToggle") handleMemoryToggleClick();
  if (actionBtn.id === "mathStudioToggle") handleMathStudioToggleClick();
  if (actionBtn.id === "accountToggle") handleAccountToggleClick();
  if (actionBtn.id === "thinkingToggle") handleThinkingToggleClick();
  if (actionBtn.id === "thinkingPromoClose") handleThinkingPromoCloseClick();
  if (actionBtn.id === "clearMemory") handleClearMemoryClick();
  if (actionBtn.id === "solveGeometryBtn") handleSolveGeometryClick();
  if (actionBtn.id === "mathTutorNext") handleMathTutorNextClick();
  if (actionBtn.id === "mathTutorSkip") handleMathTutorSkipClick();
  if (actionBtn.id === "mathTutorDone") handleMathTutorDoneClick();
  if (actionBtn.id === "drawerClose") handleDrawerCloseClick();
  if (actionBtn.id === "drawerAccountSettings") handleDrawerAccountSettingsClick();
  if (actionBtn.id === "newChatBtn") handleNewChatClick();
  if (actionBtn.id === "drawerPremiumOpen") handleDrawerPremiumOpenClick();
  if (actionBtn.id === "drawerBackgroundOpen") handleDrawerBackgroundOpenClick();
  if (actionBtn.id === "backgroundClose") handleBackgroundCloseClick();
  if (actionBtn.id === "premiumClose") handlePremiumCloseClick();
  if (actionBtn.id === "saveAccount") handleSaveAccountClick();
  if (actionBtn.id === "persistBrowserAccount") handlePersistAccountClick();
  if (actionBtn.id === "logoutAccount") handleLogoutAccountClick();
  if (actionBtn.id === "deleteAccount") handleDeleteAccountClick();
  if (actionBtn.id === "lensClose") handleLensCloseClick();
  if (actionBtn.id === "lensPickBtn") handleLensPickClick();
  if (actionBtn.id === "lensAnalyzeBtn") handleLensAnalyzeClick();
  if (actionBtn.id === "banUnlockBtn") handleBanUnlockClick();
  if (actionBtn.id === "balukEngineBtn") openBalukEngine();
  if (actionBtn.id === "closeSafetySurveyModal") handleCloseSafetySurveyClick();
  if (actionBtn.id === "thinkingUnlockBtn") handleThinkingUnlockClick();
  if (actionBtn.id === "thinkingUpgradeBtn") handleThinkingUpgradeClick();
  if (actionBtn.id === "thinkingDismissBtn") handleThinkingDismissClick();
  if (actionBtn.id === "uiSettingsToggle") handleUiSettingsOpenClick();
  if (actionBtn.id === "uiSettingsClose") handleUiSettingsCloseClick();
  if (actionBtn.id === "uiModeLightBtn") setScopedUiMode("app", "light");
  if (actionBtn.id === "uiModeDarkBtn") setScopedUiMode("app", "dark");
  if (actionBtn.id === "engineModeLightBtn") setScopedUiMode("engine", "light");
  if (actionBtn.id === "engineModeDarkBtn") setScopedUiMode("engine", "dark");
  if (actionBtn.id === "premiumBuyBtn") handlePremiumBuyClick();
  if (actionBtn.id === "premiumPayLinkBtn") handlePremiumPayLinkClick();
  if (actionBtn.id === "premiumConfirmBtn") handlePremiumConfirmClick();
  if (actionBtn.id === "engineBackBtn") handleEngineBackClick();
  if (actionBtn.id === "engineThemeToggle") handleEngineThemeClick();
  if (actionBtn.id === "engineAuthLoginBtn") handleEngineAuthLoginClick();
  if (actionBtn.id === "engineModelPicker") handleEngineModelPickerClick();
  if (actionBtn.id === "engineLanguagePicker") handleEngineLanguagePickerClick();
  if (actionBtn.id === "engineIdeasToggle") handleEngineIdeasToggleClick();
  if (actionBtn.id === "engineCodeToggle") handleEngineCodeToggleClick();
  if (actionBtn.id === "engineRunToggle") handleEngineRunToggleClick();
  if (actionBtn.id === "engineSubmitBtn") handleEngineSubmitClick();
  if (actionBtn.id === "engineDockClose") handleEngineDockCloseClick();
  if (actionBtn.id === "engineSettingsClose") handleEngineSettingsCloseClick();
  if (actionBtn.id === "engineQuotaUpgradeBtn") handleEngineQuotaUpgradeClick();
  if (actionBtn.id === "engineQuotaDismissBtn") handleEngineQuotaDismissClick();
  if (actionBtn.id === "voiceCloseBtn") handleVoiceCloseClick();
  if (actionBtn.id === "voiceWebBtn") handleVoiceWebClick();
  if (actionBtn.id === "voiceMuteBtn") handleVoiceMuteClick();
}, true);
document.addEventListener("change", (e) => {
  const control = e.target.closest("#advancedMathMode, #webSearchMode, #balukLensMode, #testModeToggle, #allowProfanityMode, #backgroundThemeSelect, #backgroundMusicSelect, #engineIdeasCheckbox");
  if (!control) return;
  e.stopPropagation();
  if (control.id === "advancedMathMode") setAdvancedMathMode(advancedMathMode?.checked);
  if (control.id === "webSearchMode") setWebMode(webSearchMode?.checked);
  if (control.id === "balukLensMode") setLensMode(balukLensMode?.checked);
  if (control.id === "testModeToggle") setTestMode(testModeToggle?.checked);
  if (control.id === "allowProfanityMode") {
    if (!isPremiumUser) {
      allowProfanity = false;
      if (allowProfanityMode) allowProfanityMode.checked = false;
      showWarningOverlay("Bu özellik premium üyelik gerektirir.");
      return;
    }
  ...
  if (control.id === "backgroundThemeSelect") applyBackgroundTheme(backgroundThemeSelect?.value);
  if (control.id === "backgroundMusicSelect") setBackgroundMusic(backgroundMusicSelect?.value);
  if (control.id === "engineIdeasCheckbox") toggleEngineIdeas(engineIdeasCheckboxEl?.checked);
}, true);
document.addEventListener("input", (e) => {
  const control = e.target.closest("#backgroundMusicVolume");
  if (!control) return;
  e.stopPropagation();
  setBackgroundVolume(backgroundMusicVolume?.value);
}, true);
if (chatForm && userInput) chatForm.addEventListener("submit", (e) => {
  e.preventDefault();
...
if (modelToggle && modelMenu) modelToggle.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopImmediatePropagation();
  if (!isAccountLoggedIn) {
    showWarningOverlay("Model seçmek için önce oturum aç.");
...
document.addEventListener("click", (e) => {
  if (modelMenu && modelToggle && !modelMenu.contains(e.target) && !modelToggle.contains(e.target)) modelMenu.classList.add("hidden");
...
initGeometryLab();
updateSplashPrompt();
ensureDrawerSettingsButton();
ensureDrawerBalosButton();
ensureAgentModeButton();
...
function openBalukEngine() {
  if (!BALUK_ENGINE_ENABLED) return;
  engineModeOpen = true;
  if (plusMenu) plusMenu.classList.add("hidden");
...
const uiSettingsToggleBtn = document.getElementById("uiSettingsToggle");
const uiSettingsModalEl = document.getElementById("uiSettingsModal");
const uiSettingsCloseBtn = document.getElementById("uiSettingsClose");
const uiModeLightBtnEl = document.getElementById("uiModeLightBtn");
const uiModeDarkBtnEl = document.getElementById("uiModeDarkBtn");
const engineThemeToggleBtn = document.getElementById("engineThemeToggle");
const engineSettingsPanelEl = document.getElementById("engineSettingsPanel");
const engineSettingsCloseBtn = document.getElementById("engineSettingsClose");
const engineModeLightBtnEl = document.getElementById("engineModeLightBtn");
const engineModeDarkBtnEl = document.getElementById("engineModeDarkBtn");
const engineAuthGateEl = document.getElementById("engineAuthGate");
const engineAuthPhraseEl = document.getElementById("engineAuthPhrase");
const engineAuthLoginBtnEl = document.getElementById("engineAuthLoginBtn");
const engineMainLayoutEl = document.getElementById("engineMainLayout");
const engineRunToggleBtn = document.getElementById("engineRunToggle");
const engineQuotaFillEl = document.getElementById("engineQuotaFill");
const engineIdeasToggleBtn = document.getElementById("engineIdeasToggle");
const engineIdeasCheckboxEl = document.getElementById("engineIdeasCheckbox");
const engineIdeasSettingEl = document.getElementById("engineIdeasSetting");
const engineQuotaPanelEl = document.getElementById("engineQuotaPanel");
const engineQuotaUpgradeBtnEl = document.getElementById("engineQuotaUpgradeBtn");
const engineQuotaDismissBtnEl = document.getElementById("engineQuotaDismissBtn");

const UI_MODE_STORAGE_KEY = "balukUiMode";
const ENGINE_UI_MODE_STORAGE_KEY = "balukEngineUiMode";
const ENGINE_QUOTA_STORAGE_KEY = "balukEngineQuota";
const ENGINE_IDEAS_STORAGE_KEY = "balukEngineIdeas";
const ENGINE_CODE_TAB_STORAGE_KEY = "balukEngineCodeTab";
const ENGINE_AUTH_PHRASES = [
  "</Hadi bir sey uretelim>",
  "</Bir fikir kuralim>",
  "</HTML ile bir sey yazalim>",
  "</Baluk Engine ile baslayalim>",
  "</Bugun ne gelistirelim?>"
];
const ENGINE_BRAIN_ICON = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
  <path d="M9 4.5C7.067 4.5 5.5 6.067 5.5 8C4.119 8.552 3.25 9.89 3.25 11.375C3.25 12.75 4 13.95 5.125 14.575C5.042 14.85 5 15.14 5 15.5C5 17.433 6.567 19 8.5 19H10V4.9C9.682 4.633 9.359 4.5 9 4.5Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
  <path d="M15 4.5C16.933 4.5 18.5 6.067 18.5 8C19.881 8.552 20.75 9.89 20.75 11.375C20.75 12.75 20 13.95 18.875 14.575C18.958 14.85 19 15.14 19 15.5C19 17.433 17.433 19 15.5 19H14V4.9C14.318 4.633 14.641 4.5 15 4.5Z" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>
  <path d="M10 8C8.9 8 8 8.9 8 10" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
  <path d="M14 8C15.1 8 16 8.9 16 10" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
  <path d="M10 12C8.9 12 8 12.9 8 14" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
  <path d="M14 12C15.1 12 16 12.9 16 14" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"/>
</svg>`;

let appUiMode = localStorage.getItem(UI_MODE_STORAGE_KEY) || "light";
let engineUiMode = localStorage.getItem(ENGINE_UI_MODE_STORAGE_KEY) || "dark";
let engineIdeasEnabled = localStorage.getItem(ENGINE_IDEAS_STORAGE_KEY) !== "0";
let engineCodeTab = ["html", "css", "js"].includes(localStorage.getItem(ENGINE_CODE_TAB_STORAGE_KEY))
  ? localStorage.getItem(ENGINE_CODE_TAB_STORAGE_KEY)
  : "html";
let engineAuthPhraseTimer = null;
...
function setStoredEngineQuota(remaining = 100) {
  if (!Number.isFinite(remaining)) return;
  localStorage.setItem(ENGINE_QUOTA_STORAGE_KEY, JSON.stringify({
    day: getTodayQuotaKey(),
    remaining: clamp(Math.round(remaining), 0, 100)
  }));
}

function getStoredEngineQuota() {
  if (isPremiumUser) {
    return { day: getTodayQuotaKey(), remaining: Infinity };
  }
  let payload = null;
  try {
    payload = JSON.parse(localStorage.getItem(ENGINE_QUOTA_STORAGE_KEY) || "null");
  } catch {}
  const today = getTodayQuotaKey();
  if (!payload || payload.day !== today || !Number.isFinite(payload.remaining)) {
    payload = { day: today, remaining: 100 };
    setStoredEngineQuota(100);
  }
  return payload;
}

function setScopedUiMode(scope = "app", mode = "light") {
  const safeMode = mode === "dark" ? "dark" : "light";
  if (scope === "engine") {
    engineUiMode = safeMode;
    localStorage.setItem(ENGINE_UI_MODE_STORAGE_KEY, safeMode);
  } else {
    appUiMode = safeMode;
...
function syncVisibleUiMode() {
  const activeMode = engineModeOpen ? engineUiMode : appUiMode;
  document.body.classList.toggle("ui-light-mode", activeMode === "light");
  document.body.classList.toggle("ui-dark-mode", activeMode === "dark");
  syncModeButtons();
}

function syncModeButtons() {
  if (uiModeLightBtnEl) uiModeLightBtnEl.classList.toggle("active", appUiMode === "light");
  if (uiModeDarkBtnEl) uiModeDarkBtnEl.classList.toggle("active", appUiMode === "dark");
  if (engineModeLightBtnEl) engineModeLightBtnEl.classList.toggle("active", engineUiMode === "light");
  if (engineModeDarkBtnEl) engineModeDarkBtnEl.classList.toggle("active", engineUiMode === "dark");
}

function openUiSettingsModal() {
  if (!uiSettingsModalEl) return;
  uiSettingsModalEl.classList.remove("hidden");
  syncModeButtons();
}

function closeUiSettingsModal() {
  if (uiSettingsModalEl) uiSettingsModalEl.classList.add("hidden");
}

function openEngineSettingsPanel() {
  if (!engineSettingsPanelEl) return;
  syncEngineIdeasUi();
  engineSettingsPanelEl.classList.remove("hidden");
  syncModeButtons();
}

function closeEngineSettingsPanel() {
  if (engineSettingsPanelEl) engineSettingsPanelEl.classList.add("hidden");
}

function startEngineAuthPhraseLoop() {
  if (!engineAuthPhraseEl) return;
  stopEngineAuthPhraseLoop();
  let index = 0;
  engineAuthPhraseEl.textContent = ENGINE_AUTH_PHRASES[index];
  engineAuthPhraseTimer = window.setInterval(() => {
    index = (index + 1) % ENGINE_AUTH_PHRASES.length;
    engineAuthPhraseEl.textContent = ENGINE_AUTH_PHRASES[index];
  }, 2200);
}

function stopEngineAuthPhraseLoop() {
  if (!engineAuthPhraseTimer) return;
  window.clearInterval(engineAuthPhraseTimer);
  engineAuthPhraseTimer = null;
}

function refreshEngineAccessState() {
  const locked = !isAccountLoggedIn;
  if (engineAuthGateEl) engineAuthGateEl.classList.toggle("hidden", !locked);
  if (engineMainLayoutEl) engineMainLayoutEl.classList.toggle("hidden", locked);
  if (locked) startEngineAuthPhraseLoop();
  else stopEngineAuthPhraseLoop();
}

function createEngineChatSession(initialTitle = "Bir sey uretelim") {
  return {
    id: `engine_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    title: initialTitle,
    entries: [],
...
function normalizeEngineSession(session) {
  return {
    ...createEngineChatSession(session?.title || "Bir sey uretelim"),
    ...session,
    entries: Array.isArray(session?.entries) ? session.entries : [],
    model: session?.model === "thinking" ? "thinking" : "fast",
    language: "html",
    code: typeof session?.code === "string" ? session.code : "",
    outputHtml: typeof session?.outputHtml === "string" ? session.outputHtml : ""
  };
...
function ensureActiveEngineSession() {
  if (!engineChatSessions.length) {
    const base = createEngineChatSession();
    engineChatSessions = [base];
    activeEngineChatId = base.id;
  }
  if (!getActiveEngineSession()) activeEngineChatId = engineChatSessions[0]?.id || null;
  const active = getActiveEngineSession();
  if (active) Object.assign(active, normalizeEngineSession(active));
...
function saveEngineSessions() {
  localStorage.setItem(ENGINE_CHAT_STORAGE_KEY, JSON.stringify({
    activeEngineChatId,
    sessions: engineChatSessions.slice(0, 1).map((session) => normalizeEngineSession(session))
  }));
  localStorage.setItem(ENGINE_PANEL_STORAGE_KEY, enginePanelMode || "");
}
...
function restoreEngineSessions() {
  ...
  try {
    const parsed = JSON.parse(localStorage.getItem(ENGINE_CHAT_STORAGE_KEY) || "null");
    if (Array.isArray(parsed?.sessions) && parsed.sessions.length) {
      nextSession = normalizeEngineSession(parsed.sessions[0]);
      activeEngineChatId = parsed.activeEngineChatId || nextSession.id;
    }
  } catch {}
  engineChatSessions = [nextSession || createEngineChatSession()];
  activeEngineChatId = activeEngineChatId || engineChatSessions[0].id;
  const savedPanel = localStorage.getItem(ENGINE_PANEL_STORAGE_KEY) || "";
  enginePanelMode = ["code", "run"].includes(savedPanel) ? savedPanel : null;
  ensureActiveEngineSession();
}
...
function setEngineStatus(title, text, processing = false) {
  engineStatusStateTitle = title || "Baluk Engine";
  engineStatusStateText = text || "";
  if (engineModeOpen) renderEngineProjectView();
}
...
function appendEngineEntry(role, text, meta = "", kind = "") {
  ...
  active.entries.push({ role, text, meta, kind, at: Date.now() });
  active.updatedAt = Date.now();
  if (role === "user" && (!active.title || active.title === "Bir sey uretelim")) {
    active.title = makeChatTitleFromText(text);
  }
  saveEngineSessions();
}
...
function updateEngineSelectionMeta() {
  const active = ensureActiveEngineSession();
  const quota = getStoredEngineQuota();
  const quotaLabel = isPremiumUser ? "%∞" : `%${quota.remaining}`;
  if (engineSelectionMeta) engineSelectionMeta.textContent = `Hiz: ${active.model === "thinking" ? "Thinking" : "Fast"} • Dil: HTML • Hak: ${quotaLabel}`;
}

function updateEngineThinkingQuota() {
  if (!engineThinkingQuota || !engineQuotaFillEl) return;
  const quota = getStoredEngineQuota();
  if (isPremiumUser) {
    engineThinkingQuota.textContent = "%∞";
    engineQuotaFillEl.style.width = "100%";
    engineQuotaFillEl.parentElement?.parentElement?.classList.add("is-premium");
    return;
  }
  const value = clamp(Number(quota.remaining || 0), 0, 100);
  engineThinkingQuota.textContent = `%${value}`;
  engineQuotaFillEl.style.width = `${value}%`;
  engineQuotaFillEl.parentElement?.parentElement?.classList.remove("is-premium");
}

function restoreEnginePickerIcons() {
  if (engineModelPicker) engineModelPicker.innerHTML = `<span class="engine-diamond-btn-text">${ENGINE_BRAIN_ICON}</span>`;
  if (engineLanguagePicker) engineLanguagePicker.innerHTML = `<span class="engine-diamond-btn-text">&lt;&gt;</span>`;
}

function syncEngineIdeasUi() {
  const active = ensureActiveEngineSession();
  const show = active.model === "thinking";
  if (engineIdeasToggleBtn) {
    engineIdeasToggleBtn.classList.toggle("hidden", !show);
    engineIdeasToggleBtn.classList.toggle("active", show && engineIdeasEnabled);
    engineIdeasToggleBtn.textContent = show
      ? (engineIdeasEnabled ? "Kendi fikirleri acik" : "Kendi fikirleri kapali")
      : "Kendi fikirleri";
  }
  if (engineIdeasSettingEl) engineIdeasSettingEl.classList.toggle("hidden", !show);
  if (engineIdeasCheckboxEl) engineIdeasCheckboxEl.checked = engineIdeasEnabled;
}

function toggleEngineIdeas(forceValue) {
  const active = ensureActiveEngineSession();
  if (active.model !== "thinking") return;
  engineIdeasEnabled = typeof forceValue === "boolean" ? forceValue : !engineIdeasEnabled;
  localStorage.setItem(ENGINE_IDEAS_STORAGE_KEY, engineIdeasEnabled ? "1" : "0");
  syncEngineIdeasUi();
  updateEngineSelectionMeta();
}

function closeEngineQuotaPanel() {
  if (engineQuotaPanelEl) engineQuotaPanelEl.classList.add("hidden");
}

function openEngineQuotaPanel() {
  if (engineQuotaPanelEl) engineQuotaPanelEl.classList.remove("hidden");
}
...
function cleanEnginePhrase(text = "") {
  return String(text || "")
    .trim()
    .replace(/^["'“”‘’]+|["'“”‘’]+$/g, "")
    .replace(/\s{2,}/g, " ");
...
function detectColorToken(text = "") {
  const lower = normalizeEnginePrompt(text);
  if (/(mavi|blue)/.test(lower)) return "#2563eb";
  if (/(yesil|green)/.test(lower)) return "#16a34a";
  if (/(kirmizi|red)/.test(lower)) return "#dc2626";
  if (/(mor|purple)/.test(lower)) return "#7c3aed";
  if (/(turuncu|orange)/.test(lower)) return "#ea580c";
  if (/(sari|yellow)/.test(lower)) return "#ca8a04";
  if (/(siyah|black)/.test(lower)) return "#111827";
  if (/(beyaz|white)/.test(lower)) return "#ffffff";
  return "";
}

function extractTextAfterPatterns(prompt = "", patterns = []) {
  const raw = String(prompt || "");
  for (const pattern of patterns) {
    const match = raw.match(pattern);
    if (match?.[1]) {
      const value = cleanEnginePhrase(match[1]);
...
function extractEngineText(prompt = "", fallback = "") {
  const raw = String(prompt || "");
  const direct = extractTextAfterPatterns(raw, [
    /["“']([^"”']{1,80})["”']\s+yaz(?:isi|ısı)?\s+ekle/i,
    /(.{1,80}?)\s+yaz(?:isi|ısı)?\s+ekle/i,
    /ekrana\s+(.{1,80}?)\s+yaz/i,
    /basliga?\s+(.{1,80}?)\s+yaz/i
  ]);
  if (direct) return direct;
  const quoted = raw.match(/["“']([^"”']{1,80})["”']/);
  if (quoted?.[1]) return cleanEnginePhrase(quoted[1]);
  return fallback;
}

function extractEngineButtonLabel(prompt = "", fallback = "Buton") {
  const raw = String(prompt || "");
  const label = extractTextAfterPatterns(raw, [
    /ustunde\s+["“']?(.{1,40}?)["”']?\s+yazan\s+(?:bir\s+)?buton/i,
    /ustunde\s+["“']?(.{1,40}?)["”']?\s+yazsin/i,
    /buton(?:un)?\s+ustundeki\s+yaziyi\s+["“']?(.{1,40}?)["”']?\s+yap/i,
    /buton(?:un)?\s+yazisini\s+["“']?(.{1,40}?)["”']?\s+yap/i
  ]);
  return label || fallback;
}

function extractClickText(prompt = "") {
  return extractTextAfterPatterns(prompt, [
    /bas(?:inca|ınca|ildiginda|ıldığında).*?["“']?(.{1,60}?)["”']?\s+yaz(?:isi|ısı)?\s+ciksin/i,
    /bas(?:inca|ınca|ildiginda|ıldığında).*?["“']?(.{1,60}?)["”']?\s+yazsin/i,
    /tikla(?:yinca|yınca).*?["“']?(.{1,60}?)["”']?\s+yaz(?:isi|ısı)?\s+ciksin/i,
    /tikla(?:yinca|yınca).*?["“']?(.{1,60}?)["”']?\s+yazsin/i
  ]);
}

function extractBackgroundColor(prompt = "") {
  const raw = String(prompt || "");
  const match = raw.match(/(?:arka\s*plani?|background)[^.,;\n]*?(mavi|kirmizi|kırmızı|yesil|yeşil|mor|turuncu|sari|sarı|beyaz|siyah|blue|red|green|purple|orange|yellow|white|black)/i);
  return detectColorToken(match?.[1] || "");
}

function extractButtonColor(prompt = "") {
  const raw = String(prompt || "");
  const match = raw.match(/buton[^.,;\n]*?(mavi|kirmizi|kırmızı|yesil|yeşil|mor|turuncu|sari|sarı|beyaz|siyah|blue|red|green|purple|orange|yellow|white|black)/i);
  return detectColorToken(match?.[1] || "");
}

function extractTextColor(prompt = "") {
  const raw = String(prompt || "");
  const match = raw.match(/(?:yazi|yazı|metin|baslik|başlık)[^.,;\n]*?(mavi|kirmizi|kırmızı|yesil|yeşil|mor|turuncu|sari|sarı|beyaz|siyah|blue|red|green|purple|orange|yellow|white|black)/i);
  return detectColorToken(match?.[1] || "");
}

function extractClickBackgroundColor(prompt = "") {
  const raw = String(prompt || "");
  const match = raw.match(/(?:bas(?:inca|ınca)|tikla(?:yinca|yınca))[^.,;\n]*?(mavi|kirmizi|kırmızı|yesil|yeşil|mor|turuncu|sari|sarı|beyaz|siyah|blue|red|green|purple|orange|yellow|white|black)[^.,;\n]*?(?:arka\s*plan|background)/i)
    || raw.match(/(?:bas(?:inca|ınca)|tikla(?:yinca|yınca))[^.,;\n]*?(?:arka\s*plan|background)[^.,;\n]*?(mavi|kirmizi|kırmızı|yesil|yeşil|mor|turuncu|sari|sarı|beyaz|siyah|blue|red|green|purple|orange|yellow|white|black)/i);
  return detectColorToken(match?.[1] || "");
}

function analyzeEnginePrompt(prompt = "") {
  const raw = String(prompt || "").trim();
  const lower = normalizeEnginePrompt(raw);
  const textValue = extractEngineText(raw, "");
  const buttonLabel = extractEngineButtonLabel(raw, "Buton");
  const clickText = extractClickText(raw);
  const backgroundColor = extractBackgroundColor(raw);
  const buttonColor = extractButtonColor(raw);
  const textColor = extractTextColor(raw);
  const clickBackgroundColor = extractClickBackgroundColor(raw);
  const addText = /(yazisi ekle|yazısı ekle|ekrana .* yaz|baslik ekle|başlık ekle|baslik olustur|başlık oluştur)/.test(lower);
  const addButton = /(buton|button)/.test(lower) && /(ekle|olustur|oluştur|koy)/.test(lower);
  const addInput = /(input|giris|giriş|form)/.test(lower) && /(ekle|olustur|oluştur|koy)/.test(lower);
  const centerText = /(ortala|ortalanmis|ortalanmış|merkezde|ortada)/.test(lower);
  const growText = /(buyut|büyüt|buyuk|büyük|32px|40px|48px|56px|64px)/.test(lower);
  const roundButton = /(kose.*yuvarlak|köşe.*yuvarlak|koseleri yuvarlak|köşeleri yuvarlak|yuvarlak yap|rounded)/.test(lower);
  const growButton = /(buton.*buyut|buton.*büyük|buton.*buyuk|butonu buyut|butonu büyük|butonu buyuk|biraz buyuk)/.test(lower);
  const addAnimation = /(animasyon|animation|hover)/.test(lower);
  const renameButton = /(buton(?:un)? .*yazisini .*yap|buton(?:un)? .*yaziyi .*yap)/.test(lower) ? buttonLabel : "";
  const clickShowText = /(basinca|basınca|tiklayinca|tıklayınca|tıkla)/.test(lower) && !!clickText;
  const clickChangeBackground = /(basinca|basınca|tiklayinca|tıklayınca)/.test(lower) && !!clickBackgroundColor;
  const addHeading = /(baslik|başlık)/.test(lower);
  const actions = [];
  if (backgroundColor) actions.push("background");
  if (addText) actions.push("addText");
  if (centerText) actions.push("centerText");
  if (growText) actions.push("growText");
  if (textColor) actions.push("textColor");
  if (addButton) actions.push("addButton");
  if (buttonColor) actions.push("buttonColor");
  if (roundButton) actions.push("roundButton");
  if (growButton) actions.push("growButton");
  if (renameButton) actions.push("renameButton");
  if (clickShowText) actions.push("clickText");
  if (clickChangeBackground) actions.push("clickBackground");
  if (addAnimation) actions.push("animation");
  if (addInput) actions.push("input");
  return {
    raw,
    lower,
    textValue,
...
function computeEnginePromptCost(prompt = "", model = "fast") {
  const analysis = analyzeEnginePrompt(prompt);
  const unit = model === "thinking" ? 5 : 2;
  const actionCount = Math.max(1, analysis.actions.length || 1);
  return {
    ...analysis,
    actionCount,
    cost: isPremiumUser ? 0 : actionCount * unit
  };
}

function serializeEngineDoc(doc) {
  return `<!DOCTYPE html>\n${doc.documentElement.outerHTML}`;
}

function parseEngineHtml(code = "") {
  const parser = new DOMParser();
  return parser.parseFromString(ensureBaseHtmlCode(code), "text/html");
}

function mergeInlineStyle(node, styleText = "") {
  if (!node || !styleText.trim()) return;
  const current = node.getAttribute("style") || "";
  const separator = current.trim() && !current.trim().endsWith(";") ? "; " : "";
  node.setAttribute("style", `${current}${separator}${styleText}`.trim());
}

function getLastEngineNode(doc, type = "text") {
  return [...doc.body.querySelectorAll(`[data-engine-type="${type}"]`)].pop() || null;
}

function appendEngineScript(doc, content = "") {
...
function applyThinkingIdeasPolish(doc, analysis) {
  if (!engineIdeasEnabled) return;
  mergeInlineStyle(doc.body, "margin:0;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:18px;font-family:'Segoe UI',Arial,sans-serif;padding:24px;");
  if (!analysis.backgroundColor) {
    mergeInlineStyle(doc.body, "background:linear-gradient(180deg,#140f21,#0f172a);");
  }
  const lastText = getLastEngineNode(doc, "text");
  const lastButton = getLastEngineNode(doc, "button");
  if (lastText && !analysis.textColor) {
    mergeInlineStyle(lastText, "color:#f8fafc;");
  }
  if (lastButton) {
    mergeInlineStyle(lastButton, "border:none;padding:14px 26px;border-radius:18px;background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#ffffff;font-weight:700;cursor:pointer;box-shadow:0 16px 28px rgba(76,29,149,.28);transition:transform .18s ease, box-shadow .18s ease;");
    const style = doc.createElement("style");
    style.textContent = `button[data-engine-type="button"]:hover{transform:translateY(-2px);box-shadow:0 20px 36px rgba(76,29,149,.34);}`;
...
function buildIncrementalHtmlArtifact(prompt = "", advanced = false, existingCode = "") {
  const analysis = analyzeEnginePrompt(prompt);
  const doc = parseEngineHtml(existingCode);
  const notes = [];
  ...
  if (!doc.title) doc.title = "Baluk Engine";

  if (analysis.backgroundColor) {
    mergeInlineStyle(doc.body, `background:${analysis.backgroundColor};`);
    if (!/min-height/i.test(doc.body.getAttribute("style") || "")) mergeInlineStyle(doc.body, "min-height:100vh;");
    notes.push("arka plani guncelledim");
  }

  let lastText = getLastEngineNode(doc, "text");
  if (analysis.addText) {
    const element = doc.createElement(analysis.addHeading || analysis.growText ? "h1" : "p");
    element.setAttribute("data-engine-type", "text");
    element.textContent = analysis.textValue || "Metin";
    if (analysis.textColor) mergeInlineStyle(element, `color:${analysis.textColor};`);
    if (analysis.centerText) mergeInlineStyle(element, "text-align:center;width:100%;");
    if (analysis.growText) mergeInlineStyle(element, `font-size:${advanced ? "56px" : "38px"};`);
    doc.body.appendChild(element);
    lastText = element;
    notes.push(`"${element.textContent}" yazisini ekledim`);
  } else if (lastText) {
    if (analysis.textColor) {
      mergeInlineStyle(lastText, `color:${analysis.textColor};`);
      notes.push("son yazinin rengini degistirdim");
    }
    if (analysis.centerText) {
      mergeInlineStyle(lastText, "text-align:center;width:100%;");
      notes.push("son yaziyi ortaladim");
    }
    if (analysis.growText) {
      mergeInlineStyle(lastText, `font-size:${advanced ? "56px" : "38px"};`);
      notes.push("son yaziyi buyuttum");
    }
  }

  let lastButton = getLastEngineNode(doc, "button");
  if (analysis.addButton) {
    const buttonId = nextEngineElementId(existingCode || serializeEngineDoc(doc), "engineButton");
    const button = doc.createElement("button");
    button.id = buttonId;
    button.setAttribute("data-engine-type", "button");
    button.textContent = analysis.buttonLabel || "Buton";
    if (analysis.buttonColor) mergeInlineStyle(button, `background:${analysis.buttonColor};color:${analysis.buttonColor === "#ffffff" ? "#111827" : "#ffffff"};`);
    if (analysis.roundButton) mergeInlineStyle(button, "border-radius:18px;");
    if (analysis.growButton) mergeInlineStyle(button, "padding:14px 24px;font-size:18px;");
    if (analysis.addAnimation) mergeInlineStyle(button, "transition:transform .18s ease, box-shadow .18s ease;");
    doc.body.appendChild(button);
    lastButton = button;
    notes.push(`"${button.textContent}" butonunu ekledim`);
  } else if (lastButton) {
    if (analysis.renameButton) {
      lastButton.textContent = analysis.renameButton;
      notes.push("son butonun yazisini guncelledim");
    }
    if (analysis.buttonColor) {
      mergeInlineStyle(lastButton, `background:${analysis.buttonColor};color:${analysis.buttonColor === "#ffffff" ? "#111827" : "#ffffff"};`);
      notes.push("son butonun rengini degistirdim");
    }
    if (analysis.roundButton) {
      mergeInlineStyle(lastButton, "border-radius:18px;");
      notes.push("son butonun koselerini yuvarladim");
    }
    if (analysis.growButton) {
      mergeInlineStyle(lastButton, "padding:14px 24px;font-size:18px;");
      notes.push("son butonu buyuttum");
    }
    if (analysis.addAnimation) {
      mergeInlineStyle(lastButton, "transition:transform .18s ease, box-shadow .18s ease;");
      notes.push("son butona animasyon hazirladim");
    }
  }

  if (analysis.clickShowText && lastButton?.id) {
    const textColor = analysis.textColor || detectColorToken(analysis.raw) || "#2563eb";
    appendEngineScript(doc, `document.getElementById("${lastButton.id}")?.addEventListener("click", function () {
  const yazi = document.createElement("p");
  yazi.textContent = "${escapeJsString(analysis.clickText)}";
  yazi.style.color = "${textColor}";
  yazi.style.margin = "12px 0 0";
  this.insertAdjacentElement("afterend", yazi);
});`);
    notes.push(`butona tiklaninca "${analysis.clickText}" yazisi cikiyor`);
  }

  if (analysis.clickChangeBackground && lastButton?.id) {
    appendEngineScript(doc, `document.getElementById("${lastButton.id}")?.addEventListener("click", function () {
  document.body.style.background = "${analysis.clickBackgroundColor}";
});`);
    notes.push("butona tiklaninca arka plan degisiyor");
  }

  if (analysis.addInput) {
    const input = doc.createElement("input");
    input.type = "text";
    input.placeholder = "Buraya yaz...";
    input.setAttribute("data-engine-type", "input");
    doc.body.appendChild(input);
    notes.push("bir input ekledim");
  }

  if (advanced) {
    mergeInlineStyle(doc.body, "margin:0;font-family:'Segoe UI',Arial,sans-serif;");
    if (engineIdeasEnabled) applyThinkingIdeasPolish(doc, analysis);
  }

  const code = serializeEngineDoc(doc);
  return {
    summary: notes.length ? `Tamam, ${notes.join(" ve ")}.` : "Komutunu anladim ama koda yeni bir sey eklemedim.",
    code,
    outputHtml: code
  };
}

function splitEngineHtmlSections(code = "") {
  const safeCode = ensureBaseHtmlCode(code);
  const css = [...safeCode.matchAll(/<style[^>]*>([\s\S]*?)<\/style>/gi)].map((match) => match[1].trim()).filter(Boolean).join("\n\n");
  const js = [...safeCode.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)].map((match) => match[1].trim()).filter(Boolean).join("\n\n");
  const html = safeCode
    .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "")
    .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "")
    .trim();
  return {
    html: html || ensureBaseHtmlCode(""),
    css: css || "/* CSS henuz yok */",
    js: js || "// JavaScript henuz yok"
  };
}
...
function renderHighlightedCode(code = "", language = "html") {
  ...
  if (language === "css") {
    const highlighted = escaped
      .replace(/(\/\*[\s\S]*?\*\/)/g, '<span class="engine-syntax-comment">$1</span>')
      .replace(/([.#]?[a-zA-Z0-9_-]+\s*\{)/g, '<span class="engine-syntax-tag">$1</span>')
      .replace(/([a-z-]+)(\s*:)/gi, '<span class="engine-syntax-attr">$1</span>$2')
      .replace(/(#(?:[0-9a-fA-F]{3,8}))/g, '<span class="engine-syntax-number">$1</span>');
    return `<pre class="engine-code-block engine-code-block-highlight"><code>${highlighted}</code></pre>`;
  }
  if (language === "javascript" || language === "js") {
    const highlighted = escaped
      .replace(/(\/\/.*)$/gm, '<span class="engine-syntax-comment">$1</span>')
      .replace(/(&quot;.*?&quot;|'.*?')/g, '<span class="engine-syntax-string">$1</span>')
      .replace(/\b(const|let|var|function|return|if|else|document|addEventListener|createElement|appendChild|style)\b/g, '<span class="engine-syntax-keyword">$1</span>')
      .replace(/\b([0-9]+)\b/g, '<span class="engine-syntax-number">$1</span>');
    return `<pre class="engine-code-block engine-code-block-highlight"><code>${highlighted}</code></pre>`;
  }
  if (language === "python") {
...
function buildEngineArtifact(prompt = "", language = "html", model = "fast", existingCode = "") {
  return buildIncrementalHtmlArtifact(prompt, model === "thinking", existingCode);
}

function getEngineStatusSteps(prompt = "", model = "fast") {
  const analysis = analyzeEnginePrompt(prompt);
  const steps = [];
  if (analysis.addText) steps.push({ thinking: "Yaziyi ekliyorum...", done: `Tamam, "${analysis.textValue || "metin"}" yazisini ekledim.` });
  if (analysis.growText) steps.push({ thinking: "Yaziyi buyutuyorum...", done: "Tamam, yaziyi buyuttum." });
  if (analysis.centerText) steps.push({ thinking: "Yaziyi ortaliyorum...", done: "Tamam, yaziyi ortaladim." });
  if (analysis.addButton) steps.push({ thinking: "Butonu ekliyorum...", done: `Tamam, "${analysis.buttonLabel || "Buton"}" butonunu ekledim.` });
  if (analysis.roundButton || analysis.growButton || analysis.buttonColor || analysis.renameButton) {
    steps.push({ thinking: "Butonu duzenliyorum...", done: "Tamam, son butonu guncelledim." });
  }
  if (analysis.clickShowText || analysis.clickChangeBackground) {
    steps.push({ thinking: "Tiklama davranisini hazirliyorum...", done: "Tamam, tiklama davranisini ekledim." });
  }
  if (analysis.backgroundColor) steps.push({ thinking: "Arka plani ayarliyorum...", done: "Tamam, arka plani duzenledim." });
  if (analysis.addInput) steps.push({ thinking: "Inputu ekliyorum...", done: "Tamam, inputu ekledim." });
  if (!steps.length) steps.push({ thinking: "Kod iskeletini hazirliyorum...", done: "Tamam, temel kodu hazirladim." });
  if (model === "thinking") {
    steps.push({
      thinking: engineIdeasEnabled ? "Kodu guzellestirip son kontrolleri yapiyorum..." : "Son kontrolleri yapiyorum...",
      done: engineIdeasEnabled ? "Tamam, ufak dokunuslarla kodu daha iyi hale getirdim." : "Tamam, son kontrolleri bitirdim."
    });
  }
  return steps;
}

function estimateEngineThinkingMs(prompt = "", model = "fast") {
  const actionCount = Math.max(1, analyzeEnginePrompt(prompt).actions.length || 1);
  const base = model === "thinking" ? 20000 : 10000;
  const extra = String(prompt || "").trim().length * (model === "thinking" ? 45 : 22) + actionCount * (model === "thinking" ? 1800 : 1200);
  const max = model === "thinking" ? 40000 : 20000;
  const min = model === "thinking" ? 20000 : 10000;
  return clamp(Math.round(base + extra), min, max);
}

function isEngineResetPrompt(prompt = "") {
  const lower = normalizeEnginePrompt(prompt);
  return /(kodu sil|her seyi sil|her şeyi sil|bastan basla|baştan başla|sifirdan basla|sıfırdan başla|reset)/.test(lower);
}

async function runEngineStatuses(statuses = [], totalMs = 12000) {
  const delay = Math.max(2400, Math.floor(totalMs / Math.max(1, statuses.length)));
  for (const step of statuses) {
    setEngineStatus("Baluk Engine dusunuyor", step.thinking || "Kod planlaniyor...", true);
    renderEngineProjectView();
    await new Promise((resolve) => setTimeout(resolve, delay));
    appendEngineEntry("assistant", step.done || "Tamam.", "Adim", "step");
    renderEngineProjectView();
  }
}
...
function renderEngineDock() {
  if (!engineDock || !engineDockTitle || !engineDockBody) return;
  const active = ensureActiveEngineSession();
  if (engineCodeToggle) engineCodeToggle.classList.toggle("active", enginePanelMode === "code");
  if (engineRunToggleBtn) engineRunToggleBtn.classList.toggle("active", enginePanelMode === "run");
  engineDock.classList.toggle("hidden", !enginePanelMode);
  if (!enginePanelMode) return;
  if (enginePanelMode === "run") {
    engineDockTitle.textContent = "Onizleme";
    engineDockBody.innerHTML = active.code
      ? `<iframe class="engine-preview-frame" title="Baluk Engine onizleme" sandbox="allow-scripts" srcdoc="${escapeHtml(active.code)}"></iframe>`
      : `<div class="engine-project-empty"><p>Onizleme icin once kod uret.</p></div>`;
    return;
  }

  const sections = splitEngineHtmlSections(active.code);
  localStorage.setItem(ENGINE_CODE_TAB_STORAGE_KEY, engineCodeTab);
  engineDockTitle.textContent = "Kod";
  engineDockBody.innerHTML = `
    <div class="engine-code-tabs">
      <button class="engine-code-tab${engineCodeTab === "html" ? " active" : ""}" type="button" data-engine-tab="html">HTML</button>
      <button class="engine-code-tab${engineCodeTab === "css" ? " active" : ""}" type="button" data-engine-tab="css">CSS</button>
      <button class="engine-code-tab${engineCodeTab === "js" ? " active" : ""}" type="button" data-engine-tab="js">JS</button>
    </div>
    <section class="engine-code-section${engineCodeTab === "html" ? " active" : ""}" data-engine-section="html">
      <div class="engine-code-actions"><button class="engine-code-copy" type="button" data-copy="html">HTML kopyala</button></div>
      ${renderHighlightedCode(sections.html, "html")}
    </section>
    <section class="engine-code-section${engineCodeTab === "css" ? " active" : ""}" data-engine-section="css">
      <div class="engine-code-actions"><button class="engine-code-copy" type="button" data-copy="css">CSS kopyala</button></div>
      ${renderHighlightedCode(sections.css, "css")}
    </section>
    <section class="engine-code-section${engineCodeTab === "js" ? " active" : ""}" data-engine-section="js">
      <div class="engine-code-actions"><button class="engine-code-copy" type="button" data-copy="js">JS kopyala</button></div>
      ${renderHighlightedCode(sections.js, "javascript")}
    </section>
  `;
  engineDockBody.querySelectorAll("[data-engine-tab]").forEach((btn) => {
    btn.addEventListener("click", () => {
      engineCodeTab = btn.getAttribute("data-engine-tab") || "html";
      renderEngineDock();
    });
  });
  engineDockBody.querySelectorAll("[data-copy]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const key = btn.getAttribute("data-copy") || "html";
      const text = key === "css" ? sections.css : key === "js" ? sections.js : sections.html;
      try {
...
function renderEngineProjectView() {
  ...
  if (!entries.length && !engineRunning) {
    engineProjectView.innerHTML = `<div class="engine-project-empty"><p>Baluk Engine hazir. Istedigini yazdiginda burada konusacak, dusunecek ve kodu olusturacak.</p></div>`;
    return;
  }
  const feedHtml = entries.map((entry) => `
    <article class="engine-conversation-bubble ${entry.role === "user" ? "user" : "assistant"}${entry.kind === "step" ? " thinking" : ""}">
      <strong>${entry.role === "user" ? "Sen" : "Baluk Engine"}</strong>
      <p>${escapeHtml(entry.text || "")}</p>
      ${entry.meta ? `<span>${escapeHtml(entry.meta)}</span>` : ""}
    </article>
  `).join("");
  const thinkingHtml = engineRunning ? `
    <article class="engine-conversation-bubble assistant thinking">
      <strong>Baluk Engine</strong>
      <div class="engine-thinking-card">
        <div class="engine-thinking-actor">
          <div class="engine-thinking-fish-wrap">
            <svg class="engine-fish" viewBox="0 0 520 220" fill="none" xmlns="http://www.w3.org/2000/svg"><use href="#baluk-fish"></use></svg>
            <div class="engine-bubbles"><span>&lt;/&gt;</span><span>&lt;/&gt;</span><span>&lt;/&gt;</span></div>
          </div>
          <div class="engine-thinking-copy">
            <strong>${escapeHtml(engineStatusStateTitle || "Baluk Engine dusunuyor")}</strong>
            <p>${escapeHtml(engineStatusStateText || "Kod planlaniyor...")}</p>
          </div>
        </div>
      </div>
    </article>
  ` : "";
  engineProjectView.innerHTML = `<div class="engine-conversation-feed">${feedHtml}${thinkingHtml}</div>`;
  engineProjectView.scrollTop = engineProjectView.scrollHeight;
}

function renderEngineWorkspace() {
  if (!engineWorkspace) return;
  ensureActiveEngineSession();
  restoreEnginePickerIcons();
  refreshEngineAccessState();
  syncEngineIdeasUi();
  renderEngineProjectView();
  renderEngineDock();
  updateEngineSelectionMeta();
  ...
  if (engineModelMenu) {
    engineModelMenu.querySelectorAll("[data-engine-model]").forEach((btn) => {
      btn.classList.toggle("active", btn.getAttribute("data-engine-model") === ensureActiveEngineSession().model);
    });
  }
  if (engineLanguageMenu) {
    engineLanguageMenu.querySelectorAll("[data-engine-language]").forEach((btn) => {
      btn.classList.toggle("active", btn.getAttribute("data-engine-language") === "html");
    });
  }
  if (enginePrompt) enginePrompt.placeholder = "Bir sey uretelim";
}
...
function selectEngineModel(model) {
  const active = ensureActiveEngineSession();
  active.model = model === "thinking" ? "thinking" : "fast";
  active.updatedAt = Date.now();
  saveEngineSessions();
  syncEngineIdeasUi();
  updateEngineSelectionMeta();
...
function openBalukEngine() {
  ...
  if (engineWorkspace) {
    engineWorkspace.classList.add("engine-enter");
    setTimeout(() => engineWorkspace.classList.remove("engine-enter"), 600);
  }
  if (appRoot) appRoot.classList.add("engine-mode-active");
  syncVisibleUiMode();
  refreshEngineAccessState();
  setEngineStatus("Baluk Engine hazir", "Komut ver, kodu burada olusturayim.");
  renderEngineWorkspace();
}
...
async function submitEnginePrompt() {
  ...
  if (isEngineResetPrompt(prompt)) {
    ...
    active.outputHtml = "";
    active.lastPrompt = "";
    active.entries = active.entries.slice(-1);
    appendEngineEntry("assistant", "Tamam, Baluk Engine icindeki kodu temizledim. Yeni komutunu bekliyorum.", "Baluk Engine");
    saveEngineSessions();
    enginePrompt.value = "";
    setEngineStatus("Baluk Engine hazir", "Temiz bir projeyle devam edebiliriz.");
    renderEngineWorkspace();
    return;
  }

  const costInfo = computeEnginePromptCost(prompt, active.model);
  if (!isPremiumUser) {
    const quota = getStoredEngineQuota();
    if (quota.remaining < costInfo.cost) {
      openEngineQuotaPanel();
      setEngineStatus("Hak doldu", "Yeni komut icin hak barinin dolmasini bekle ya da yukselt.");
      renderEngineWorkspace();
      return;
    }
  }

  appendEngineEntry("user", prompt, `${active.model === "thinking" ? "Thinking" : "Fast"} • HTML`);
  active.lastPrompt = prompt;
  active.updatedAt = Date.now();
  enginePrompt.value = "";
  engineRunning = true;
  ...
  const statuses = getEngineStatusSteps(prompt, active.model);
  await runEngineStatuses(statuses, estimateEngineThinkingMs(prompt, active.model));

  const artifact = buildEngineArtifact(prompt, "html", active.model, active.code || "");
  active.code = artifact.code;
  active.outputHtml = artifact.outputHtml || artifact.code;
  active.updatedAt = Date.now();
  if (!isPremiumUser) {
    const quota = getStoredEngineQuota();
    setStoredEngineQuota(quota.remaining - costInfo.cost);
  }
  engineRunning = false;
  appendEngineEntry(
    "assistant",
    `${artifact.summary} Code ile HTML/CSS/JS bolumlerini gorebilir, Run ile onizlemeyi acabilirsin.`,
    active.model === "thinking" ? (engineIdeasEnabled ? "Thinking • Kendi fikirleri acik" : "Thinking") : "Fast"
  );
  setEngineStatus(
    active.model === "thinking" ? "Thinking tamamlandi" : "Fast tamamlandi",
    active.model === "thinking" ? "Daha guclu bir HTML paketi hazirlandi." : "Temel HTML paketi hazirlandi."
  );
  saveEngineSessions();
  renderEngineWorkspace();
}

function bindClickOutsideClose(panel, closeFn) {
  if (!panel) return;
  panel.addEventListener("click", (event) => {
    if (event.target === panel) closeFn();
  });
}

function initUiSettingsControls() {
  if (window.__uiSettingsControlsInit) return;
  window.__uiSettingsControlsInit = true;
  if (uiSettingsToggleBtn) uiSettingsToggleBtn.addEventListener("click", openUiSettingsModal);
  if (uiSettingsCloseBtn) uiSettingsCloseBtn.addEventListener("click", closeUiSettingsModal);
  bindClickOutsideClose(uiSettingsModalEl, closeUiSettingsModal);
}

function replaceBalukEngineButton() {
  if (!BALUK_ENGINE_ENABLED) return;
  const currentBtn = document.getElementById("balukEngineBtn");
  if (!currentBtn) return;
  const clone = currentBtn.cloneNode(true);
  currentBtn.replaceWith(clone);
...
function initBalukEngineV2() {
  if (!BALUK_ENGINE_ENABLED) return;
  if (window.__balukEngineV2Init) return;
  window.__balukEngineV2Init = true;
  restoreEngineSessions();
  replaceBalukEngineButton();
  restoreEnginePickerIcons();
  setTimeout(restoreEnginePickerIcons, 0);

  if (uiSettingsToggleBtn) uiSettingsToggleBtn.addEventListener("click", openUiSettingsModal);
  if (uiSettingsCloseBtn) uiSettingsCloseBtn.addEventListener("click", closeUiSettingsModal);
  if (uiModeLightBtnEl) uiModeLightBtnEl.addEventListener("click", () => setScopedUiMode("app", "light"));
  if (uiModeDarkBtnEl) uiModeDarkBtnEl.addEventListener("click", () => setScopedUiMode("app", "dark"));
  bindClickOutsideClose(uiSettingsModalEl, closeUiSettingsModal);

  if (engineThemeToggleBtn) engineThemeToggleBtn.addEventListener("click", openEngineSettingsPanel);
  if (engineSettingsCloseBtn) engineSettingsCloseBtn.addEventListener("click", closeEngineSettingsPanel);
  if (engineModeLightBtnEl) engineModeLightBtnEl.addEventListener("click", () => setScopedUiMode("engine", "light"));
  if (engineModeDarkBtnEl) engineModeDarkBtnEl.addEventListener("click", () => setScopedUiMode("engine", "dark"));
  bindClickOutsideClose(engineSettingsPanelEl, closeEngineSettingsPanel);

  if (engineRunToggleBtn) engineRunToggleBtn.addEventListener("click", () => toggleEnginePanel("run"));
  if (engineIdeasToggleBtn) engineIdeasToggleBtn.addEventListener("click", () => toggleEngineIdeas());
  if (engineIdeasCheckboxEl) engineIdeasCheckboxEl.addEventListener("change", () => toggleEngineIdeas(engineIdeasCheckboxEl.checked));

  if (engineAuthLoginBtnEl) {
    engineAuthLoginBtnEl.addEventListener("click", () => {
      closeBalukEngine();
      if (accountPanel) accountPanel.classList.remove("hidden");
      if (sideDrawer) sideDrawer.classList.add("hidden");
    });
  ...
  if (engineQuotaUpgradeBtnEl) {
    engineQuotaUpgradeBtnEl.addEventListener("click", () => {
      closeEngineQuotaPanel();
      if (premiumModal) premiumModal.classList.remove("hidden");
    });
  }
  if (engineQuotaDismissBtnEl) engineQuotaDismissBtnEl.addEventListener("click", closeEngineQuotaPanel);
  bindClickOutsideClose(engineQuotaPanelEl, closeEngineQuotaPanel);

  syncVisibleUiMode();
  renderEngineWorkspace();
}

window.__balukEngineV2Ready = true;
